...
import { UPLOAD_VIDEO_ENABLED } from '../constants.js';
    ...
    <div className="stage-empty">
      ...
      >
        ...
        <div className="alt-sources">
          <label
            className={`source-button${dragDropActive ? ' is-drag-target' : ''}${UPLOAD_VIDEO_ENABLED ? '' : ' is-disabled'}`}
            aria-label={UPLOAD_VIDEO_ENABLED ? 'Upload video file' : 'Upload video file (coming soon)'}
          >
            <input
              type="file"
              accept="video/*,.mov,.mp4,.webm"
              onChange={handleFile}
              disabled={videoLoading || !UPLOAD_VIDEO_ENABLED}
            />
            ...
            Upload video
            {!UPLOAD_VIDEO_ENABLED && <span className="coming-soon-badge">Coming soon</span>}
          </label>
          <button
            type="button"
